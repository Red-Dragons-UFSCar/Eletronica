...
typedef struct struct_message {
  ...
  //  Vetor da mensagem
  uint8_t RD[lengthVector] = {111, 170, 160, 0, 0, 0, 0, 58, 58, 112};
  int cont = 0;
} struct_message;
...
volatile double KpA = 1.944;
volatile double KiA = 69.18;
volatile double KpB = 1.293;
volatile double KiB = 71.15;
...
volatile int SaidaA = 0;
volatile int SaidaB =0;
int pwm_a;
int pwm_b;
const double Soma_A = KpA+(KiA/100);
const double Soma_B = KpB+(KiB/100);
...
void IRAM_ATTR onTime()
{
  ...
  if (myData.dataAvaliable) {
    ...
    volatile int Anterior = KpA*ErroAnterior1A;
    volatile int Anterior2 = (ErroAtualA*Soma_A);
    SaidaA = Anterior2 - Anterior + SaidaA;
    ...
    volatile int Anterior2B = (ErroAtualB*Soma_A);
    SaidaB = Anterior2B - AnteriorB + SaidaB;
    
    //SaidaB = (ErroAtualB*(KpB + KiB*dt)) - KpB*ErroAnterior1B + SaidaB;
    ...
    pwm_b = map(SaidaB,0,70,120,255);
    pwm_a = map(SaidaA,0,70,120,255);
    pwm_a = max(pwm_a,0);
    pwm_a = min(pwm_a,255);
    pwm_b = max(pwm_b,0);
    pwm_b = min(pwm_b,255);
    ...
    motor(pwm_a,pwm_b);
      ...
      if(ROBOT == 1){
          KpA = myData.RD[3]*(pow(10,Escala_P*(-1)));
          KiA = myData.RD[4]*(pow(10,Escala_P*(-1)));
      } else if (ROBOT ==2){
          KpA = myData.RD[5]*(pow(10,Escala_P*(-1)));
          KiA = myData.RD[6]*(pow(10,Escala_P*(-1)));
      } else if(ROBOT ==3){
          KpA = myData.RD[7]*(pow(10,Escala_P*(-1)));
          KiA = myData.RD[8]*(pow(10,Escala_P*(-1)));
...
//    esp_err_t result = esp_now_send(broadcastAddress, (uint8_t *) &ReturnData, sizeof(ReturnData));
  ...
  if (true){
    ...
    Serial.print(SaidaB);
    Serial.print(" ");
    Serial.print(ErroAtualA);
    Serial.print(" ");
    Serial.println(ErroAtualB);
  }
}