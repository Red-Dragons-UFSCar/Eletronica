...
typedef struct struct_message {
  ...
  uint8_t RD[lengthVector] = {111, 170, 160, 10, 10, 10, 10, 10, 10, 113};
} struct_message;
...
volatile int ErroAnterior2A = 0;
volatile int ErroAnterior1B = 0;
volatile int ErroAnterior2B = 0;
int ErroAtualA = 0;
int ErroAtualB = 0;
...
volatile double KpA = 1.1;
volatile double KiA = 50;
volatile double KpB = 1.1;
volatile double KiB = 50;
volatile double KdA = 0.1/dt;
volatile double KdB = 0.1/dt;
...
volatile double Anterior3B = 0;
volatile double AnteriorA = 0;
volatile double Anterior2A = 0;
volatile double Anterior3A = 0;
...
void IRAM_ATTR onTime()
{
  ...
  if (myData.dataAvaliable) {
    ...
    //Erro 
    ErroAtualA = ref_a/1.7409 - counterAB;
    counterAB = 0;
  ...
  /*  
    if(compA > 50){
      ...
      Anterior3A = (ErroAtualA - 2*ErroAnterior1A + ErroAnterior2A)*KdA;
      //SaidaA = Anterior2A - AnteriorA + SaidaAnteriorA;
      SaidaA = Anterior2A - AnteriorA + SaidaAnteriorA + Anterior3A;
    ...
    */
    ...
    Anterior2A = (ErroAtualA*Soma_A);
    SaidaA = Anterior2A - AnteriorA + SaidaAnteriorA;
    SaidaA = (SaidaA + 4*SaidaAnteriorA)/4;
 ...
 /*
    if(compB > 50){
      ...
      Anterior3B = (ErroAtualB - 2*ErroAnterior1B + ErroAnterior2B)*KdB;
      SaidaB = Anterior2B - AnteriorB + SaidaAnteriorB + Anterior3B;
    }
       valB=contB*19;
       if(SaidaB >= valB){
       SaidaB = valB;
  ...
  */
    ...
    SaidaB = (SaidaB + 4*SaidaAnteriorB)/4;

    
    ErroAnterior1A = ErroAtualA;
    ErroAnterior2A = ErroAnterior1A;
    ErroAnterior1B = ErroAtualB;
    ErroAnterior2B = ErroAnterior1B;
    SaidaAnteriorA = SaidaA;
    SaidaAnteriorB = SaidaB;

    if(SaidaAnteriorA == 0){
    pwm_b = map(SaidaB,0,60,150,255);
    pwm_a = map(SaidaA,0,60,150,255); 
    }
    else{
    pwm_b = map(SaidaB,0,60,60,255);
    pwm_a = map(SaidaA,0,60,60,255); 
    }
