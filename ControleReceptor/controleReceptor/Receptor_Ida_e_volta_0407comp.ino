...
void IRAM_ATTR onTime()
{
  ...
  if (myData.dataAvaliable) {
    ...
    //Erro 
    ErroAtualA = ref_a - counterAB;
    counterAB = 0;
    ErroAtualA = min(ErroAtualA, 30);
    ErroAtualB = ref_a - counterAB;
    counterAB_b = 0;
    ErroAtualB = min(ErroAtualB, 30);
    SaidaA += (ErroAtualA*(KpA + KiA*dt) - ErroAnterior1A*(KpA));
    if(SaidaA > 70){
      SaidaA = 70;
    }
    SaidaB += (ErroAtualB*(KpB + KiB*dt) - ErroAnterior1B*(KpB));
    if(SaidaB > 70){
      SaidaB = 70;
    }
